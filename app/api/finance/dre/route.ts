...
export async function GET(req: Request) {
    try {
        ...
        const [
            ...
            supabaseAdmin.from('sales').select('total').eq('tenant_id', tenant.id).gte('created_at', startOfDay(new Date(start)).toISOString()).lte('created_at', endOfDay(new Date(end)).toISOString())
        ]);
        ...
        const salesRevenue = salesData?.reduce((acc, curr) => acc + Number(curr.total), 0) || 0;
