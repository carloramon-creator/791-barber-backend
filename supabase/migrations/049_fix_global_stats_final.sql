-- Migration 049_fix_global_stats_final.sql
-- Fix get_system_global_stats to use correctly named columns in 'sales' table
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'in_progress_revenue', (SELECT coalesce(sum(total), 0) FROM sales WHERE paid = false)
    ) INTO result;
...
COMMENT ON FUNCTION get_system_global_stats() IS 'Retorna estatísticas globais do SaaS usando as colunas corretas (total, paid) da tabela sales.';
